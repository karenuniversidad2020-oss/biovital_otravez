...
class AuthController {
    ...
    public function login() {
        ...
        if (!empty($login->objetos)) {
            ...
            if ($this->isAjax()) {
                ApiResponse::send(true, ApiResponse::CODE_SUCCESS, 'Inicio de sesión exitoso', ['redirect' => $redirectUrl], 200);
            } else {
            ...
            // Login fallido
            if ($this->isAjax()) {
                ApiResponse::send(false, ApiResponse::CODE_AUTH_ERROR, 'Cédula o contraseña incorrecta', [], 200);
            } else {
                redirect('?error=1');
            }
        }
