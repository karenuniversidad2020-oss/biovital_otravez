...
function setupSpecialtyCards() {
    var specialtyInfo = {
        general: { title: 'Medicina General', description: 'Atención integral para adultos y niños, diagnóstico y tratamiento de enfermedades comunes.', services: ['Consulta general', 'Chequeos preventivos', 'Vacunación', 'Exámenes de rutina'] },
        cardiologia: { title: 'Cardiología', description: 'Especializada en diagnóstico y tratamiento de enfermedades del corazón.', services: ['Electrocardiograma', 'Ecocardiograma', 'Prueba de esfuerzo', 'Cateterismo'] },
        neumonologia: { title: 'Neumología', description: 'Tratamiento de enfermedades respiratorias y pulmonares.', services: ['Espirometría', 'Broncoscopía', 'Pruebas de alergia', 'Tratamiento del asma'] },
        psicologia: { title: 'Psicología', description: 'Apoyo emocional y mental para mejorar tu bienestar.', services: ['Terapia individual', 'Terapia de pareja', 'Psicoterapia', 'Evaluación psicológica'] },
        pediatria: { title: 'Pediatría', description: 'Cuidado especializado para niños desde el nacimiento hasta la adolescencia.', services: ['Control de crecimiento', 'Vacunación infantil', 'Chequeos pediátricos', 'Urgencias'] }
    };
    ...
    document.querySelectorAll('.specialty-card').forEach(function (card) {
        card.addEventListener('click', function () {
            ...
            if (info) {
                ...
                body.innerHTML = '<h3>' + info.title + '</h3><p>' + info.description + '</p><h4>Servicios:</h4><ul>' + info.services.map(function (s) { return '<li>' + s + '</li>'; }).join('') + '</ul>';
