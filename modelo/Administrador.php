...
=======
    ...
    function listarUsuarios($rol = null, $busqueda = null) {
        try {
            ...
            // Función auxiliar para agregar usuarios de una tabla
            $agregarUsuarios = function($tabla, $tipo, $id_field, $nombre_field, $apellido_field, $cedula_field, $telefono_field, $correo_field) use (&$usuarios, $rol, $busqueda) {
                if ($rol !== null && $rol !== $tipo) {
                    return;
                }
                
                $sql = "SELECT 
                            $id_field as id,
                            $nombre_field as nombre,
                            $apellido_field as apellidos,
                            $cedula_field as cedula,
                            $telefono_field as telefono,
                            $correo_field as correo,
                            :tipo as tipo
                        FROM $tabla
                        WHERE 1=1";
                ...
                if ($busqueda) {
                    $sql .= " AND ($nombre_field LIKE :busqueda 
                              OR $apellido_field LIKE :busqueda 
                              OR $cedula_field LIKE :busqueda)";
                    $params[':busqueda'] = "%$busqueda%";
                }
                
                $sql .= " ORDER BY $nombre_field ASC LIMIT 100";
            ...
            // Agregar pacientes (tipo 1)
            $agregarUsuarios('registro_paciente', 'paciente', 'id_paciente', 
                            'nombre_paciente', 'apellido_paciente', 
                            'cedula_paciente', 'telefono_paciente', 'correo_paciente');
            
            // Agregar médicos (tipo 2)
            $agregarUsuarios('registro_medico', 'medico', 'id_medico', 
                            'nombre_medico', 'apellido_medico', 
                            'cedula_medico', 'telefono_medico', 'correo_medico');
            
            // Agregar asistentes (tipo 3)
            $agregarUsuarios('registro_asistente', 'asistente', 'id_asistente', 
                            'nombre_asistente', 'apellido_asistente', 
                            'cedula_asistente', 'telefono_asistente', 'correo_asistente');
            
            // Agregar administradores (tipo 4)
            $agregarUsuarios('registro_administrador', 'administrador', 'id_administrador', 
